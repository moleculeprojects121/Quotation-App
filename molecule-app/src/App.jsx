...
function NotificationModal({ title, text, onClose }) {
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl p-5 max-w-xs w-full mx-3 shadow-2xl border border-slate-200 text-center">
        ...
        <p className="text-slate-500 text-[11px] mb-4">{text}</p>
...
function ScopePopup({ deliverables, strategy, onClose, onApply }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl max-w-xl w-full mx-3 shadow-2xl border border-slate-200 flex flex-col max-h-[90vh]">
        ...
        <div className="p-3 border-t border-slate-100 bg-slate-50 rounded-b-xl flex justify-between items-center">
          <button onClick={() => { setSelected({}); setCustomDesc(''); setCustomRate('') }} className="text-[10px] text-slate-400 hover:text-red-500 font-bold uppercase tracking-wider">Reset Selection</button>
          <div className="flex space-x-2">
            ...
            <button onClick={handleApply} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg shadow-sm flex items-center transition-colors">
              <i className="fa-solid fa-check mr-1.5"></i> Add selected to calculations
            </button>
          </div>
        </div>
      </div>
...
function NestedItemPopup({ parentId, parentDesc, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl max-w-sm w-full mx-3 shadow-2xl border border-slate-200 flex flex-col">
...
function PrintableReport({ clientName, projectName, sqft, strategy, deliverables, milestones, invitationText, setInvitationText }) {
  ...
  return (
    <div className="border border-slate-300 bg-white rounded-lg shadow-lg max-w-2xl mx-auto text-slate-800 relative overflow-hidden" id="printableReportArea">
      <div className="brand-banner-bg h-3 w-full"></div>
      <div className="p-5">
        {/* Letterhead */}
        ...
        <div className="relative group my-3">
          <div contentEditable suppressContentEditableWarning
            ...
            className="text-[10px] text-slate-600 leading-relaxed outline-none border border-transparent hover:border-slate-300 focus:border-blue-500 p-1.5 rounded transition-all cursor-pointer">
            {invitationText}
          </div>
          ...
          <table className="w-full text-left text-[10px] border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-bold">
                <th className="py-1 px-2.5">Consultancy Stage / Deliverables</th>
                <th className="py-1 px-2.5 text-right">Rate</th>
                <th className="py-1 px-2.5 text-right">Area sq.ft</th>
                <th className="py-1 px-2.5 text-right">Total PKR</th>
              </tr>
            </thead>
...
// ─── Step 2: Spreadsheet ──────────────────────────────────────────────────────
function SpreadsheetView({ deliverables, setDeliverables, sqft, strategy, clientName }) {
  ...
  return (
    <>
      <div className="flex-grow overflow-auto custom-scrollbar max-h-[420px]">
        <table className="w-full text-left border-collapse text-xs" id="spreadsheetTable">
          <thead>
            <tr className="bg-slate-900 text-white uppercase tracking-wider text-[10px] font-bold sticky top-0 z-10">
              <th className="py-2 px-3 w-8 text-center">Sel</th>
              <th className="py-2 px-2 w-12 text-center">Code</th>
              <th className="py-2 px-3">Design Deliverable &amp; Fit-Out Scope Packages</th>
              <th className="py-2 px-3 w-24 text-right">Rate/Sq.Ft (PKR)</th>
              <th className="py-2 px-3 w-24 text-right">Area sq. ft.</th>
              <th className="py-2 px-3 w-28 text-right">Total (PKR)</th>
              <th className="py-2 px-3 w-20 text-center">Tax %</th>
              <th className="py-2 px-3 w-28 text-right">Total PKR with Tax</th>
              <th className="py-2 px-2 w-10 text-center">Rem</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {filtered.map(item => {
              if (item.isParent) { lastParentId = item.id; groupSeq = 0 }
              else groupSeq++
              const rate = strategy === 'optimal' ? item.optimalRate : item.bufferedRate
              const tax = item.taxRate ?? 15
              ...
              return (
                <tr key={item.id} className={`${item.isParent ? 'bg-slate-50 font-semibold text-slate-950 border-t border-slate-200' : 'hover:bg-slate-50/50 transition-colors'} ${item.checked ? 'row-selected' : ''}`}>
...
export default function App() {
  ...
  let statusClass = 'text-[10px] text-slate-500 font-bold'
  if (loadedVersionId) {
    statusTag = `Version ${loadedVersionId} Active (Saved)`
    statusClass = 'text-[10px] text-green-600 font-bold'
  }
  ...
  function triggerSaveVersion(forceSaveAsNew = false) {
    ...
    const snapshot = {
      ...
      sqft, strategy, subtotal, grandTotal,
    ...
    if (!forceSaveAsNew && loadedVersionId) {
      ...
      showNotif('Snapshot Version Updated', `All changes saved back to version: ${loadedVersionId}.`)
    } else {
      setSavedProposals(prev => [snapshot, ...prev])
      showNotif(`Proposal Saved as Version ${targetId}`, `Quotation archived for ${clientName || 'Unspecified Client'}.`)
  ...
  // Versions dropdown content -- show every saved version by default;
  // typing in the search box narrows it down.
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col text-slate-700 antialiased selection:bg-blue-500 selection:text-white">
      <style>{`
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1400px] mx-auto px-3 h-12 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 flex items-center justify-center bg-transparent rounded text-white select-none">
              <MoleculeLogo size={32} />
            </div>
            <div>
              <h1 className="text-sm font-bold tracking-wider text-white header-font">MOLECULE</h1>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative" ref={versionsRef}>
              ...
              {versionsOpen && (
                <div className="absolute right-0 mt-1.5 w-80 bg-white border border-slate-200 rounded-lg shadow-xl z-50 py-1 text-left">
                      ...
                      : displayedVersions.map(p => (
                        <button key={p.id} onClick={() => restoreProposal(p.clientName, p.id)}
                          ...
                          <div className="truncate pr-2">
                            ...
                            <span className="text-[8px] text-slate-400 block">{Number(p.sqft).toLocaleString()} SqFt &bull; {p.timestamp}</span>
                          </div>
      ...
      {/* Main */}
      <main className="flex-1 min-h-0 overflow-hidden max-w-[1400px] w-full mx-auto px-3 py-3 flex flex-col gap-3">
        {/* Active version banner */}
        {loadedVersionId && (
          <div className="w-full flex-shrink-0 bg-blue-50 border border-blue-200 rounded-xl p-3 flex justify-between items-center text-xs text-blue-800">
            <div className="flex items-center space-x-2">
        ...
        <div className="w-full flex-1 min-h-0 flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          {/* Step header */}
          <div className="bg-slate-50 border-b border-slate-200 p-2.5 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <h2 className="text-xs font-bold text-slate-950">
                  {currentStep === 1 ? 'Setup' : currentStep === 2 ? 'Add Scope' : 'Payment Schedule Milestones & Report Preview'}
                </h2>
              </div>
            </div>
            {/* Step dots */}
          ...
          {/* STEP 1 */}
          {currentStep === 1 && (
            <div className="p-5 flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col justify-start items-start max-w-xl mx-auto w-full space-y-4">
              <div className="w-full flex items-center justify-start pb-2 border-b border-slate-100">
              ...
              <div className="w-full bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3 shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" value={clientName} onChange={e => setClientName(e.target.value)}
                        className="w-full pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 font-medium text-xs text-slate-800"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select value={strategy} onChange={e => setStrategy(e.target.value)}
                        className="w-full pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 font-medium text-xs text-slate-800 cursor-pointer appearance-none">
                        <option value="optimal">Optimal Plan (Standard)</option>
                        <option value="buffered">Buffered Optimal Plan</option>
                      </select>
          ...
          {currentStep === 2 && (
            <div className="flex flex-col flex-1 min-h-0">
              <div className="p-2.5 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2 text-[11px]">
                <div className="flex items-center space-x-2">
              ...
              <SpreadsheetView deliverables={deliverables} setDeliverables={setDeliverables} sqft={sqft} strategy={strategy} clientName={clientName} />
            </div>
          )}
          ...
          {/* Footer */}
          <div className="mt-auto flex-shrink-0 bg-slate-50 border-t border-slate-200 p-2 px-3 flex justify-between items-center z-10">
